  ...
  return (
    <div style={{ maxWidth: 680 }}>
      <section id="the-context" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>A legacy product built for the business, not the user</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Via Benefits is Willis Towers Watson's individual benefits marketplace, used by Medicare-eligible retirees to shop for health insurance coverage. The platform handles hundreds of thousands of enrollments annually, making every friction point in the funnel a meaningful business problem.</p>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>I joined as the sole UX designer on the shopping experience team. My mandate was broad: improve the enrollment experience. What I found was a product designed entirely around how WTW had organized its insurance inventory, with no apparent consideration for how a retiree actually thinks about buying health insurance.</p>
      ...
      <section id="the-problem" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: 'var(--text-xl)', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>"They just want to see what's available for them. They don't want to answer a test question first."</p>
        </div>
      </section>

      <section id="the-research" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Research</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Diagnosing the failure with data and listening</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>I ran a dual-track research initiative. First, FullStory sessions revealed a dense cluster of rage clicks at exactly the plan type selector — users weren't just confused, they were frustrated. Second, moderated user interviews confirmed the core insight: users didn't want to select a plan category. They wanted to see what was available for someone like them.</p>
      </section>

      <section id="the-insight" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Insight</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>The product was asking users to think like the business</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The entry point was organized around WTW's product taxonomy, not around how a human being shops for insurance. The fix was clear: stop asking users what they want to buy, and start asking who they are. Age, date of birth, and zip code were all the system actually needed. Everything else was complexity we were making the user carry.</p>
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: 'var(--text-xl)', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>The system only needed three data points to do its job. We had been asking users to do that job for us.</p>
        </div>
      </section>

      <section id="the-design" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Design</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>An identity-first entry point that removed the technical gate</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The redesigned selector led with a simple question: who are you shopping for? From there, users provided age and zip code, and the system handled eligibility filtering entirely in the background. I also designed an "Escape Hatch" for users who already knew their plan type, satisfying sophisticated users without making that the default path for everyone.</p>
      </section>

      <section id="the-outcomes" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Outcomes</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Every key metric moved in the right direction</h2>
      ...
      <section>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>What I'd do differently, and what I learned</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The hardest part of this project wasn't the design. It was dismantling a belief that had calcified into institutional fact. The "Product-First mandate" everyone cited turned out to be an assumption nobody had ever verified. That's a lesson I carry into every project now: question the constraints before you design within them.</p>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>If I were doing this project again, I'd move to stakeholder alignment earlier. And I'd push harder for qualitative follow-up research after launch — the metrics told us what changed, but not how users felt about the new experience.</p>
      </section>
    </div>
  )
}
    ...
    <main>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        ...
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 'var(--text-4xl)', fontWeight: 400, lineHeight: 1.1, marginBottom: '0.5rem' }}>People-First Enrollment Redesign</h1>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        <div>
          ...
          <p className="font-serif" style={{ fontSize: 'var(--text-xl)', fontWeight: 400 }}>IHE Scheduling Portal</p>
        </div>
