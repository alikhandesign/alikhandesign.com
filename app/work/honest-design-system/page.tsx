  ...
  return (
    <div style={{ maxWidth: 680 }}>
      <section id="the-context" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>When I decided to rebuild my portfolio from scratch, the question was never what to show. The question was whether the portfolio itself would hold up as a piece of craft. Most designer portfolios are built on templates — fine for shipping quickly, not so useful when the portfolio is supposed to demonstrate exactly what you can do.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>I wanted the site to feel like me: warm but not precious, structured but not rigid, and honest about what it is. A template couldn't get there. A design system built specifically for this context could.</p>
      ...
      <section id="the-decision" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Building a design system for a portfolio is, on its face, overkill. But the alternative — designing components ad hoc and hoping they cohere — produces exactly the kind of inconsistency I spend my professional life trying to fix for other teams.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>More practically: I wanted the site's code and Figma documentation to match 1:1. That only works if the design decisions are made deliberately, captured as tokens, and implemented consistently. That's a design system, whether you call it one or not.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The name Honest came from my design philosophy: I build trust through evidence, not aesthetics. The system should be exactly what it claims to be. No decorative tokens. No components that exist for appearance only. Everything earns its place.</p>
      ...
      <section id="token-architecture" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The token architecture has two layers. Primitive tokens define the raw values: hex colors, rem sizes, numeric weights. Semantic tokens map those primitives to intent: <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>--text-muted</code> is a semantic token that resolves to <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>#6B6560</code>. Components always reference semantic tokens, never raw values.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>This separation means that if the palette changes, the swap happens in one place. It also makes Figma documentation straightforward: every token in the Figma library maps directly to a CSS custom property, by name.</p>
...
--color-bg:      var(--warm-50);`}</pre>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '2rem' }}>All spacing, typography, color, motion, and radius values are tokenized. There are no magic numbers in component code.</p>
        <CaseStudyImage
      ...
      <section id="type-and-spacing" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The type scale is rem-based with a 16px root. Every size from <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>--text-xs</code> (12px) to <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>--text-5xl</code> (48px) is a named token. This makes the type scale accessible — it inherits user browser preferences rather than overriding them — and consistent across every component.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Two typefaces: DM Serif Display for headings, Inter for UI text. The pairing was chosen for contrast and warmth. Serif headings carry weight without formality. Inter handles small sizes and data cleanly without drawing attention to itself.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Letter spacing is one of the details that most design systems underspecify. Honest has three explicit values: <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>sm</code> (0.02em, nav links), <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>md</code> (0.08em, tags and callout titles), and <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>lg</code> (0.12em, eyebrows and section labels). Each scale value is tied to a specific use case, not used interchangeably.</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem', margin: '2rem 0' }}>
          {[
            ...
            <div key={name} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', padding: '1rem' }}>
              <p style={{ fontSize: 13, fontFamily: 'monospace', color: 'var(--color-accent)', marginBottom: '0.35rem' }}>--letter-spacing-{name}</p>
              ...
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>{val} · {use}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="component-decisions" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The component inventory came from the site itself, not from a wishlist. Every component in the system has a live instance somewhere on the site. If a component was proposed during build and couldn't be placed, it was cut.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '2rem' }}>Two components are worth calling out because the decisions behind them reflect the system's philosophy:</p>
      ...
      <section id="accessibility" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Accessibility was a first-class requirement, not a post-build audit. All 35 WCAG 2.1 Level AA criteria pass. Lighthouse scores 100 on both desktop and mobile across all pages. Zero axe-reported errors. A full VoiceOver and keyboard audit was completed and documented.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>One known gap: SideNavigation does not currently implement an <code style={{ fontFamily: 'monospace', fontSize: 14, background: 'var(--color-surface-subtle)', padding: '2px 5px', borderRadius: 2 }}>aria-live</code> region for active section announcements to screen reader users. This is documented, de-prioritized (the component is decorative navigation, not primary wayfinding), and on the backlog.</p>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', margin: '2rem 0' }}>
          {[
            ...
            <div key={metric} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', padding: '1.25rem', position: 'relative', overflow: 'hidden' }}>
              ...
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.25rem' }}>{metric}</p>
              <p style={{ fontSize: 13, color: 'var(--color-text-muted)' }}>{detail}</p>
            </div>
          ))}
        </div>
        <CaseStudyImage
      ...
      <section id="figma-parity" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The Figma component library matches the production codebase 1:1. Token names, component names, prop names, and variant names are identical across both environments. This isn't just for neatness — it removes ambiguity in handoff and makes the Figma library useful as a reference for development, not just a source of screenshots.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '2rem' }}>One deliberate constraint: Figma design decisions lead code changes, not the reverse. When design specs change, code is updated to match. This preserves Figma as the source of truth and prevents the library from drifting from what's actually on the screen.</p>
        <CaseStudyImage
      ...
      <section id="the-reflection" style={{ scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Building a design system at enterprise scale means negotiating with constraints you didn't set — existing codebases, engineering capacity, legacy decisions, competing roadmap priorities. Building one for a single site means every constraint is your own. That's clarifying.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The decisions I made under pressure in larger systems — skipping token naming conventions, treating accessibility as a phase rather than a property, letting Figma and code drift — I avoided here because there was no one to negotiate with. It's easier to hold the line when you're the only person in the room.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The takeaway I'd bring back to a team: most design system debt isn't from bad decisions. It's from deferred decisions. The token architecture, the accessibility baseline, the Figma-to-code parity — none of it is hard to establish on day one. It's very hard to retrofit on day three hundred.</p>
      </section>
    </div>
  )
}
    ...
    <main>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--color-text)', fontWeight: 500 }}>Honest Design System</span>
        </nav>
        ...
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Personal · 2025–2026</p>
          <p style={{ fontSize: 18, color: 'var(--color-text)', lineHeight: 1.7, maxWidth: 680 }}>Most portfolio sites are built on templates. This one is built on a design system — 19 components, a two-layer token architecture, and a Figma library that matches production code 1:1.</p>
        </header>
      </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        ...
        <Link href="/work/ai-agent" style={{ fontSize: 14, color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
      </div>
