  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <section id="the-context" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>AI interfaces are being designed without a shared language for their failure modes</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Conversational AI products have proliferated faster than the design thinking required to evaluate them. Most UI pattern libraries were built for conventional software — deterministic systems where inputs produce predictable outputs and failure states are well-understood. AI interfaces don't behave that way. They generate rather than retrieve. They fail silently. They express uncertainty inconsistently. They require correction as a routine interaction mode rather than an edge case.</p>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>Designers working on these products are largely working without shared infrastructure. There is no publicly available, empirically grounded pattern library that treats AI-specific interaction problems as a distinct design domain. This project is an attempt to build one.</p>
      ...
      <section id="the-problem" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: 'var(--text-xl)', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>The core issue: AI output is probabilistic, but AI interfaces are designed as if it isn't. Every pattern I eventually defined traces back to this single mismatch.</p>
        </div>
      </section>

      <section id="the-framework" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Framework</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Four properties that make AI interfaces categorically different</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Before auditing any product, I needed to establish what made AI interfaces distinct enough to warrant their own pattern language. I identified four properties that separate them from conventional software interfaces.</p>
        {[
          ...
          <div key={title} style={{ marginBottom: '1.5rem', paddingLeft: '1.25rem', borderLeft: '2px solid var(--color-border)' }}>
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)', fontWeight: 600, marginBottom: '0.35rem' }}>{title}</p>
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>{body}</p>
          </div>
        ))}
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>These four properties defined the six pattern categories: Generation States, Uncertainty Communication, Source & Attribution, Limitation Handling, Correction & Refinement, and Error States.</p>
      </section>

      <section id="the-audit" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Audit</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Observed behavior across six products, using a standardized prompt set</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>I audited six products: ChatGPT, Claude, Gemini, Perplexity, Notion AI, and GitHub Copilot. The methodology was designed to produce comparable, defensible findings rather than impressionistic product reviews.</p>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>A standard prompt set of 23 prompts was written before any product was tested and applied verbatim across all six. Prompts were designed to surface behavior in each of the six pattern categories — real-time data requests to test limitation handling, ambiguous correction signals to test refinement flows, network interruption scenarios to test error states.</p>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Two data sources were treated as distinct throughout: observed product behavior and model self-assessment. Where products were asked to assess their own behavior, divergences between self-reported and observed behavior were flagged as key findings rather than used interchangeably.</p>
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: 'var(--text-xl)', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>One finding was corrected mid-audit: Claude's response to a real-time stock price request was initially characterized as a hallucination. On review, Claude had used web search and disclosed it — the correct behavior. The error traced to a gap in the audit instrument: no field existed for web search disclosure. The finding was corrected and the instrument updated. This kind of self-correction is what a rigorous methodology requires.</p>
        </div>
      </section>

      <section id="the-findings" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Findings</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--text-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>What six products revealed about the current state of AI interface design</h2>
        <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.5rem' }}>Several findings were consistent enough across products to constitute documented gaps rather than individual product failures.</p>
        {[
          ...
          <div key={title} style={{ marginBottom: '1.75rem' }}>
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)', fontWeight: 600, marginBottom: '0.5rem' }}>{title}</p>
            <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>{body}</p>
          </div>
        ))}
      </section>
      ...
      <section id="the-patterns" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <div style={{ marginTop: '2rem' }}>
          <Link href="/patterns" style={{ fontSize: 'var(--text-sm)', color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>
    ...
    <main>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text)', fontWeight: 500 }}>AI Interface Pattern Library</span>
        </nav>
        ...
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 'var(--text-4xl)', fontWeight: 400, lineHeight: 1.1, marginBottom: '0.5rem' }}>AI Interface Pattern Library</h1>
          <p style={{ fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Self-initiated · 2025–2026</p>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 3rem 3rem' }}>
        <div style={{ width: '100%', height: 400, background: 'var(--color-border)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--text-sm)', color: 'var(--color-text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>
          Hero Image
        </div>
      </div>
