    ...
    <div style={{ maxWidth: 680 }}>
      ...
      <section id="the-problem" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: '1.25rem', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>This wasn't just a process inefficiency. It was a compliance and retention risk. The voice of the user was arriving too late to matter.</p>
        </div>
      </section>
      ...
      <section id="the-reframe" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The obvious framing was: "Can AI do this faster?" But that wasn't the right question. The real problem was the Expert Gap — the mismatch between the people doing the categorization and the product knowledge required to do it accurately. A faster version of the same process would just produce wrong answers more quickly.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The right question was: "Can AI close the Expert Gap while handling sensitive healthcare data responsibly?" That reframe changed everything about how I designed the system.</p>
      </section>
      ...
      <section id="the-build" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>I started with Dataverse as the single source of truth — a structured taxonomy table grounded in WTW's specific product documentation and internal knowledge base. On top of that, two categorization approaches worked in parallel: Qualtrics Text IQ for known patterns, and Microsoft CoPilot Studio (GPT) for ambiguous cases requiring intent interpretation.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>Power Automate connected the pipeline in real time. I also built a conversational interface in Teams so stakeholders could ask direct questions — "What were the top three complaints from Medicare users this week?" — and get instant synthesized answers.</p>
      </section>
      ...
      <section id="the-validation" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>I didn't ask stakeholders to trust the AI. I built a methodology to prove it deserved trust. A double-blind accuracy audit: I manually categorized a full week of raw feedback. The AI categorized the same feedback independently. Both sets were stripped of origin labels and reviewed blind by Product Owners.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The first audit came back at 78% accuracy. Not good enough. I refined the system instructions, improved grounding queries, added fallback logic. After several iterations, accuracy reached 95% — the point at which stakeholders could no longer reliably distinguish AI categorization from expert human categorization.</p>
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: '1.25rem', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>The lead UX researcher who had told me "this will never be as good as human analysis" became one of the system's most vocal advocates.</p>
        </div>
      </section>
      ...
      <section>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The interface wasn't the hard part. The hard part was making AI trustworthy enough that people were willing to delegate important work to it. That required compliance-first thinking before the first line of code, a validation methodology rigorous enough to change a skeptic's mind, and fallback logic so humans stayed in the loop where they needed to be.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>If I were starting over, I'd build validation into day one rather than week three. The big takeaway: with agentic AI, the design challenge isn't the interface. It's building trust through validation, transparency, and continuous improvement.</p>
      </section>
    </div>
  )
}
    ...
    <main>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--color-text)', fontWeight: 500 }}>AI Feedback & Insights Agent</span>
        </nav>
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Willis Towers Watson · 2025–2026</p>
          <p style={{ fontSize: 18, color: 'var(--color-text)', lineHeight: 1.7, maxWidth: 680 }}>Nobody asked me to build this. I noticed that the research team was spending entire days doing work a well-designed system could do in minutes, and I couldn't stop thinking about what they could be doing instead.</p>
        </header>
      </div>

      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 3rem 3rem' }}>
        <div style={{ width: '100%', height: 400, background: 'var(--color-border)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>Hero Project Image</div>
      </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        ...
        <Link href="/work/people-first" style={{ fontSize: 14, color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
      </div>
