  ...
  return (
    <div style={{ maxWidth: 680 }}>
      ...
      <section id="the-research" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '1rem', marginTop: '2rem' }}>Track A — New user journey</p>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginTop: '1.5rem', marginBottom: '0.5rem' }}>Track A synthesis</p>
        <Callout>"Blueprint AI gives you a coherent website for someone like you. Not a website for you."</Callout>

        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '1rem', marginTop: '2rem' }}>Track B — Power user journey</p>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginTop: '1.5rem', marginBottom: '0.5rem' }}>Track B synthesis</p>
        <Callout>"Squarespace's AI cannot distinguish between you and anyone else — and it was never designed to."</Callout>

        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '1rem', marginTop: '2rem' }}>The comparison</p>
        ...
        <div style={{ margin: '2rem 0', padding: '1.25rem 1.5rem', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)' }}>
          <a href="https://docs.google.com/spreadsheets/d/14aYE4MwHlpsOULX9u09c1IXI_B1YytyLBPpUJ3VYBnY/edit" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', textDecoration: 'none' }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.25rem' }}>Full audit spreadsheet</p>
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>22 documented intents · user quotes · failure mode taxonomy</p>
            </div>
      ...
      <section id="the-design" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', margin: '2rem 0' }}>
          {[
            ...
            <div key={title as string} style={{ padding: '1.25rem 1.5rem', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)' }}>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.5rem' }}>{title}</p>
              <p style={{ fontSize: 14, color: 'var(--color-text-mid)', lineHeight: 1.7, marginBottom: '0.75rem' }}>{desc}</p>
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)', letterSpacing: '0.05em' }}>{modes}</p>
            </div>
          ))}
        </div>

        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.75rem', marginTop: '2.5rem' }}>Moment 1 — The Intake</p>
        <p style={{ fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: 'var(--color-accent)', fontWeight: 500, marginBottom: '0.75rem' }}>Make AI useful</p>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.75rem', marginTop: '2.5rem' }}>Moment 2 — The Transparent Builder</p>
        <p style={{ fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: 'var(--color-accent)', fontWeight: 500, marginBottom: '0.75rem' }}>Show AI reasoning</p>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.75rem', marginTop: '2.5rem' }}>Moment 3 — The Context Layer</p>
        <p style={{ fontSize: 12, letterSpacing: '0.08em', textTransform: 'uppercase' as const, color: 'var(--color-accent)', fontWeight: 500, marginBottom: '0.75rem' }}>Learn from behavior</p>
        ...
        <div style={{ margin: '2.5rem 0', padding: '1.25rem 1.5rem', background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)' }}>
          <a href="https://squarespace-ai-redesign.vercel.app" target="_blank" rel="noopener noreferrer" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', textDecoration: 'none' }}>
            <div>
              <p style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.25rem' }}>Interactive prototype</p>
              <p style={{ fontSize: 12, color: 'var(--color-text-muted)' }}>Built in React and Vercel — all three moments interactive</p>
            </div>
            <span style={{ color: 'var(--color-accent)', fontSize: 16, flexShrink: 0 }}>→</span>
          </a>
        </div>
    ...
    <main>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--color-text)', fontWeight: 500 }}>From Checkboxes to Conversations</span>
        </nav>
        ...
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Squarespace · Self-initiated · 2026</p>
          <p style={{ fontSize: 18, color: 'var(--color-text)', lineHeight: 1.7, maxWidth: 680 }}>I didn't set out to audit Squarespace's AI. I set out to redesign my portfolio. What I found instead became this case study — a documented audit of Blueprint AI across two user journeys, twenty distinct failure modes, and a redesign grounded in the principles I use every time I work with AI as a design tool.</p>
        </header>
      </div>

      {/* Hero image */}
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '0 3rem 3rem' }}>
        <div style={{ width: '100%', height: 400, background: 'var(--color-border)', borderRadius: 'var(--radius)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>Hero Image — Before/After Composite</div>
      </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        ...
        <Link href="/work/the-portfolio" style={{ fontSize: 14, color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
      </div>
