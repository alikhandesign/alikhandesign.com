  ...
  return (
    <div style={{ maxWidth: 680 }}>
      <section id="the-context" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Signify Health partners with health plans to offer eligible Medicare members a licensed clinician who comes to your home, reviews your medications and medical history, checks your vitals, and coordinates findings with your primary care provider — at no cost. The program operates at scale, with over 10,000 clinicians performing evaluations across the country. But sign-up rates weren't reflecting that potential.</p>
      ...
      <section id="the-findings" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.5rem' }}>Six distinct barrier categories emerged from the research.</p>
        {[
          ...
          <div key={title} style={{ marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid var(--color-border)' }}>
            <h3 style={{ fontSize: 14, fontWeight: 600, color: 'var(--color-text)', marginBottom: '0.4rem' }}>{title}</h3>
            <p style={{ fontSize: 16, color: 'var(--color-text-muted)', lineHeight: 1.7 }}>{text}</p>
          </div>
        ))}
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: '1.25rem', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>Members weren't afraid of the visit. They just didn't believe it was worth their time. And they had good reasons.</p>
        </div>
      </section>
      ...
      <section id="the-reframe" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The original portal was organized around what Signify Health needed from members. It assumed members already understood the value and just needed a way to book. The research said otherwise. Members needed the value case made first. Only then would they be willing to give anything back. That reframe drove every design decision in the redesign.</p>
      </section>
      ...
      <section id="the-design" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Four principles from the research shaped every design decision: lead with value not process; give members real-time scheduling control instead of a callback request; establish legitimacy early with clear trust signals; and pair every request for information with a clear explanation of what the member would receive in return.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The shift from a callback-request model to real-time self-scheduling was the most impactful single change — removing a friction point that, combined with scam skepticism, was causing significant drop-off.</p>
      </section>
      ...
      <section>
        ...
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The most valuable thing this project reinforced is that the "obvious" barrier is rarely the real one. The one-sided exchange finding was the one that stayed with me — it applies far beyond this project. Any time a product asks users to give something before they've understood what they'll receive in return, you're creating the conditions for distrust.</p>
        <p style={{ fontSize: 16, color: 'var(--color-text-mid)', lineHeight: 1.85 }}>I'd also push earlier for outcome metrics in future projects. The qualitative research here is rich and specific. A quantitative measurement of sign-up rate change before and after the redesign would have made this case study significantly stronger.</p>
      </section>
    </div>
  )
}
    ...
    <main>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--color-text)', fontWeight: 500 }}>IHE Scheduling Portal</span>
        </nav>
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Signify Health · CVS Health</p>
          <p style={{ fontSize: 18, color: 'var(--color-text)', lineHeight: 1.7, maxWidth: 680 }}>A free in-home health evaluation sounds like an obvious yes. A licensed clinician comes to your home, reviews your medications, checks your vitals, and coordinates with your doctor — at no cost. So why were millions of eligible members saying no?</p>
        </header>
      </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        ...
        <Link href="/work/squarespace-redesign" style={{ fontSize: 14, color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
      </div>
