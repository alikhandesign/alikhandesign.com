  ...
  return (
    <div style={{ maxWidth: 680 }}>
      <section id="the-context" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>A free service that millions of people were turning down</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Signify Health partners with health plans to offer eligible Medicare members a licensed clinician who comes to your home, reviews your medications and medical history, checks your vitals, and coordinates findings with your primary care provider — at no cost. The program operates at scale, with over 10,000 clinicians performing evaluations across the country. But sign-up rates weren't reflecting that potential.</p>
      ...
      <section id="the-research" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Listening before designing</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Before anyone touched a wireframe, I needed to understand what was happening in members' minds when they declined. I ran a qualitative research initiative — interviews with members who had been offered an IHE and turned it down. The sessions were structured around one core question: not "why didn't you want this?" but "help me understand what you were thinking when you made that decision." The distinction matters. The first question invites defensiveness. The second invites honesty.</p>
      </section>

      <section id="the-findings" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Findings</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>It wasn't about strangers in the home</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.5rem' }}>Six distinct barrier categories emerged from the research.</p>
        {[
        ...
        <div style={{ borderLeft: '3px solid var(--color-accent)', padding: '1.25rem 1.5rem', margin: '2rem 0', background: 'var(--color-surface)', borderRadius: '0 4px 4px 0' }}>
          <p className="font-serif" style={{ fontSize: 'var(--font-size-xl)', color: 'var(--color-text)', lineHeight: 1.5, fontStyle: 'italic' }}>Members weren't afraid of the visit. They just didn't believe it was worth their time. And they had good reasons.</p>
        </div>
      </section>

      <section id="the-reframe" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Reframe</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>From "will you let us in?" to "here's what's in it for you"</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The original portal was organized around what Signify Health needed from members. It assumed members already understood the value and just needed a way to book. The research said otherwise. Members needed the value case made first. Only then would they be willing to give anything back. That reframe drove every design decision in the redesign.</p>
      </section>

      <section id="the-design" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Design</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Trust-first, member-controlled</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>Four principles from the research shaped every design decision: lead with value not process; give members real-time scheduling control instead of a callback request; establish legitimacy early with clear trust signals; and pair every request for information with a clear explanation of what the member would receive in return.</p>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>The shift from a callback-request model to real-time self-scheduling was the most impactful single change — removing a friction point that, combined with scam skepticism, was causing significant drop-off.</p>
      </section>

      <section id="the-outcomes" style={{ marginBottom: '4rem', scrollMarginTop: '5rem' }}>
        <p className="section-label">The Outcomes</p>
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>Research that informed real scale</h2>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', margin: '2rem 0' }}>
          {[['73 NPS', 'Post-visit member satisfaction'], ['3.5M+', 'Annual IHEs completed'], ['61%', 'More likely to renew health plan coverage'], ['6', 'Distinct barrier categories identified']].map(([val, label]) => (
            <div key={label} style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', padding: '1.25rem', position: 'relative', overflow: 'hidden' }}>
              ...
              <div className="font-serif" style={{ fontSize: '2rem', color: 'var(--color-accent)', lineHeight: 1, marginBottom: '0.35rem' }}>{val}</div>
      ...
      <section>
        ...
        <h2 className="font-serif" style={{ fontSize: 'var(--font-size-base)', fontWeight: 400, lineHeight: 1.2, marginBottom: '1.25rem' }}>What the research kept teaching me</h2>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85, marginBottom: '1.25rem' }}>The most valuable thing this project reinforced is that the "obvious" barrier is rarely the real one. The one-sided exchange finding was the one that stayed with me — it applies far beyond this project. Any time a product asks users to give something before they've understood what they'll receive in return, you're creating the conditions for distrust.</p>
        <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-mid)', lineHeight: 1.85 }}>I'd also push earlier for outcome metrics in future projects. The qualitative research here is rich and specific. A quantitative measurement of sign-up rate change before and after the redesign would have made this case study significantly stronger.</p>
      </section>
    </div>
  )
}
    ...
    <main>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          ...
          <span style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text)', fontWeight: 500 }}>IHE Scheduling Portal</span>
        </nav>
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <h1 className="font-serif" style={{ fontSize: 'var(--font-size-4xl)', fontWeight: 400, lineHeight: 1.1, marginBottom: '0.5rem' }}>IHE Scheduling Portal</h1>
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>Signify Health · CVS Health</p>
          <p style={{ fontSize: 'var(--font-size-base)', color: 'var(--color-text)', lineHeight: 1.7, maxWidth: 680 }}>A free in-home health evaluation sounds like an obvious yes. A licensed clinician comes to your home, reviews your medications, checks your vitals, and coordinates with your doctor — at no cost. So why were millions of eligible members saying no?</p>
        </header>
      </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto', padding: '2.5rem 3rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap' as const, gap: '1rem' }}>
        <div>
          ...
          <p className="font-serif" style={{ fontSize: 'var(--font-size-xl)', fontWeight: 400 }}>From Checkboxes to Conversations</p>
        </div>
        <Link href="/work/squarespace-redesign" style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
      </div>
