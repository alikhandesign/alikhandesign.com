          ...
          {pillars.map((p, i) => (
            <div key={p.num} className="pillar-grid" style={{ padding: '2.5rem 0', borderTop: '1px solid var(--border)', ...(i === pillars.length - 1 ? { borderBottom: '1px solid var(--border)' } : {}) }}>
              <div>
                ...
                <h3 className="font-serif" style={{ fontSize: '1.25rem', fontWeight: 400, lineHeight: 1.2 }}>{p.title}</h3>
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', paddingTop: '0.5rem' }}>
              {aiPrinciples.map(ap => (
                <div key={ap.title} style={{ padding: '1.25rem 1.25rem 1.25rem 1.5rem', borderLeft: '3px solid var(--accent)', background: 'var(--dark-surface)', borderRadius: '0 4px 4px 0' }}>
                  <p style={{ fontSize: 14, letterSpacing: '0.06em', textTransform: 'uppercase' as const, color: 'var(--bg)', fontWeight: 600, marginBottom: '0.35rem' }}>{ap.title}</p>
                  <p style={{ fontSize: 14, color: '#C4BDB7', lineHeight: 1.6 }}>{ap.text}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="grid-2-wide" style={{ alignItems: 'start' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center' }}>
            <div style={{ background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 'var(--radius)', padding: '2rem', position: 'relative', overflow: 'hidden', width: '100%' }}>
              ...
              <h3 className="font-serif" style={{ fontSize: '1.5rem', fontWeight: 400, lineHeight: 1.2, marginBottom: '0.75rem' }}>Let's work together.</h3>
              ...
              <Link href="/work" style={{ display: 'block', marginTop: '1rem', fontSize: 14, color: 'var(--text-muted)', textDecoration: 'none' }}>View my work instead →</Link>
            </div>
          </div>
        </div>
      </section>
