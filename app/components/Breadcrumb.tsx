    ...
    <nav aria-label="Breadcrumb" style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', flexWrap: 'wrap' as const }}>
      {items.map((item, i) => (
        <span key={item.label} style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
          {i > 0 && (
            <span style={{ fontSize: 'var(--text-base)', color: 'var(--color-border-mid)' }} aria-hidden="true">›</span>
          )}
          {item.href ? (
            <Link href={item.href} style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-muted)', textDecoration: 'none' }}>
              {item.label}
            </Link>
          ) : (
