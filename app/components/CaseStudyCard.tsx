...
export default function CaseStudyCard({ title, company, tags, description, outcomes, href }: CaseStudyCardProps) {
  return (
    <Link href={href} aria-label={`View case study: ${title}`} className="work-card cs-card-grid" style={{
      ...
      <div style={{ background: 'var(--color-border)', minHeight: 220, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>
        Project Preview
      </div>
      <div style={{ padding: '2rem 2rem 2rem 2.5rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 'var(--space-8)', flexWrap: 'wrap' as const, marginBottom: 'var(--space-6)' }}>
            {outcomes.map(o => (
              <div key={o.label}>
                <div className="font-serif" style={{ fontSize: 'var(--text-2xl)', color: 'var(--color-accent)', lineHeight: 1 }}>{o.val}</div>
                <div style={{ fontSize: 'var(--text-xs)', color: 'var(--color-text-muted)', marginTop: 2, lineHeight: 1.4 }}>{o.label}</div>
              </div>
            ))}
          </div>
        </div>
