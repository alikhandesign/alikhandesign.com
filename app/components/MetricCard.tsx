...
export default function MetricCard({ company, value, description, href }: MetricCardProps) {
  ...
  return (
    ...
    >
      ...
      <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-muted)', lineHeight: 1.6, marginBottom: 'var(--space-5)' }}>{description}</p>
      <Link href={href} style={{ fontSize: 'var(--text-base)', color: 'var(--color-accent)', textDecoration: 'none', fontWeight: 'var(--font-weight-medium)' as any }}>Read case study →</Link>
    </article>
  )
}
