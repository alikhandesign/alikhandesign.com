    ...
    <>
      ...
      <nav className="site-nav" role="navigation" aria-label="Main navigation" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '1px solid var(--border)', background: 'var(--bg)', position: 'sticky', top: 0, zIndex: 100 }}>
        ...
        <ul className="nav-links" style={{ display: 'flex', listStyle: 'none' }}>
          <li><Link href="/work" style={{ textDecoration: 'none', color: path.startsWith('/work') ? 'var(--text)' : 'var(--text-muted)', fontWeight: path.startsWith('/work') ? 600 : 400, transition: 'color 0.2s' }}>My Work</Link></li>
          <li><Link href="/about" style={{ textDecoration: 'none', color: path === '/about' ? 'var(--text)' : 'var(--text-muted)', fontWeight: path === '/about' ? 600 : 400, transition: 'color 0.2s' }}>About Me</Link></li>
