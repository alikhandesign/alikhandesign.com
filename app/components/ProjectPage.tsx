    ...
    <main>
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <nav style={{ padding: '1.25rem 3rem 0', display: 'flex', alignItems: 'center', gap: '0.5rem', flexWrap: 'wrap' as const }}>
          ...
          <span style={{ fontSize: 14, color: 'var(--text)', fontWeight: 500 }}>{title}</span>
        </nav>
        <header style={{ padding: '2.5rem 3rem 3rem' }}>
          ...
          <p style={{ fontSize: 14, color: 'var(--text-muted)', letterSpacing: '0.04em', marginBottom: '1.5rem' }}>{company}</p>
      ...
      <section style={{ background: 'var(--dark-bg)', padding: '4rem 3rem' }}>
        <div style={{ maxWidth: 780, margin: '0 auto', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '2rem', flexWrap: 'wrap' as const }}>
          <div>
            ...
            <h2 className="font-serif" style={{ fontSize: '1.5rem', fontWeight: 400, color: 'var(--bg)', lineHeight: 1.25, maxWidth: 500 }}>{cta.title}</h2>
          </div>
      ...
      <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
        <div className="next-project">
          <div>
            ...
            <p className="font-serif" style={{ fontSize: '1.25rem', fontWeight: 400 }}>{next.title}</p>
          </div>
          <Link href={next.href} style={{ fontSize: 14, color: 'var(--accent)', fontWeight: 500, textDecoration: 'none' }}>View project →</Link>
        </div>
      </div>
    </main>
  )
