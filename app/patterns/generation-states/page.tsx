...
const NAV = {
  prev: { slug: 'error-states', title: 'Error States' },
  next: { slug: 'uncertainty-communication', title: 'Uncertainty Communication' },
}
      ...
      {activeTab === 'states' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
          {([
            ...
            { state: 'hung' as BadgeState, label: 'Response stalled', desc: 'Watchdog threshold exceeded. No new tokens for 5–8s.', pulse: false },
            ...
            <div key={item.state} style={{ display:'flex',alignItems:'flex-start',gap:'var(--space-6)',padding:'var(--space-4)',border:'1px solid var(--border)',borderRadius:'var(--radius)',background:'var(--surface)' }}>
              <div style={{ paddingTop: 2 }}><StatusBadge state={item.state} label={item.label} pulse={item.pulse} /></div>
              <div>
