    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 'var(--space-6)', minHeight: 160 }}>
          {genState === 'idle' && <p style={{ color: 'var(--color-text-faint)', fontSize: 'var(--font-size-sm)', fontStyle: 'italic' }}>Press a button above to simulate a generation state.</p>}
          {genState === 'thinking' && (
          ...
          {genState === 'hung' && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
              <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text)', lineHeight: 'var(--line-height-loose)' }}>{displayedText}</p>
...
function AllStates() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      {items.map(item => (
        <div key={item.state} style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-6)', padding: 'var(--space-4)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)' }}>
          <div style={{ paddingTop: 2, flexShrink: 0 }}><StatusBadge state={item.state} label={item.label} pulse={item.pulse} /></div>
          <div>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text)', fontWeight: 'var(--font-weight-medium)', marginBottom: 'var(--space-1)' }}>{item.label}</p>
