    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', background: 'var(--color-surface)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 'var(--space-3) var(--space-4)', borderBottom: '1px solid var(--color-border)', background: 'var(--color-surface-subtle)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          {versionHistory.length > 1 && (
            <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)' }}>
              ...
              <span style={{ fontSize: 'var(--color-text-xs)', color: 'var(--color-text-muted)', minWidth: 36, textAlign: 'center' }}>v{versionIndex + 1}/{versionHistory.length}</span>
        ...
        <div ref={responseRef} onMouseUp={handleMouseUp}
          ...
          <p style={{ fontSize: 'var(--color-text-base)', color: 'var(--color-text)', lineHeight: 'var(--line-height-loose)', whiteSpace: 'pre-line' }}>
            {currentResponse}
          </p>
          {selection && (
            <div style={{ position: 'absolute', top: selection.top, left: selection.left, background: 'var(--color-bg-dark)', borderRadius: 'var(--radius)', padding: '4px 6px', display: 'flex', gap: 4, zIndex: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>
              <button onClick={() => { applyRefinement('rephrase'); setSelection(null) }}
                style={{ fontSize: 'var(--color-text-xs)', color: '#fff', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', borderRadius: 2, fontFamily: 'var(--font-sans)', whiteSpace: 'nowrap' }}>
                ✍️ Rephrase selected
              </button>
              <button onClick={() => setSelection(null)}
                style={{ fontSize: 'var(--color-text-xs)', color: '#C4BDB7', background: 'none', border: 'none', cursor: 'pointer', padding: '2px 6px', borderRadius: 2, fontFamily: 'var(--font-sans)' }}>
                Flag error
              </button>
            </div>
          )}
        ...
        {demoState === 'intercepting' && (
          <div style={{ borderTop: '1px solid var(--color-border)', padding: 'var(--space-5)', background: 'var(--color-surface-subtle)' }}>
            <p style={{ fontSize: 'var(--color-text-xs)', letterSpacing: 'var(--letter-spacing-md)', textTransform: 'uppercase', color: 'var(--color-text-muted)', fontWeight: 'var(--font-weight-medium)', marginBottom: 'var(--space-4)' }}>
                  ...
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', zIndex: 10, boxShadow: '0 4px 12px rgba(0,0,0,0.08)', marginTop: 2 }}>
                    {TONE_OPTIONS.map(opt => (
                      <button key={opt} onClick={() => { setSelectedTone(opt); setShowToneDropdown(false); applyRefinement('tone', opt) }}
                        style={{ width: '100%', padding: 'var(--space-2) var(--space-4)', background: selectedTone === opt ? 'var(--color-surface-subtle)' : 'none', border: 'none', cursor: 'pointer', fontSize: 'var(--color-text-sm)', color: 'var(--color-text)', fontFamily: 'var(--font-sans)', textAlign: 'left', display: 'block' }}>
                        {opt}
                      </button>
                    ))}
                  </div>
        ...
        <div style={{ borderTop: '1px solid var(--color-border)', padding: 'var(--space-3) var(--space-4)', display: 'flex', gap: 'var(--space-3)', alignItems: 'center', background: 'var(--color-surface-subtle)' }}>
          <input value={inputValue} onChange={e => setInputValue(e.target.value)}
            ...
            style={{ flex: 1, padding: 'var(--space-2) var(--space-3)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', fontSize: 'var(--color-text-sm)', fontFamily: 'var(--font-sans)', background: 'var(--color-surface)', color: 'var(--color-text)', outline: 'none' }}
          />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      {[
        ...
        <div key={item.label} style={{ padding: 'var(--space-4)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius)', background: 'var(--color-surface)' }}>
          <p style={{ fontSize: 'var(--color-text-sm)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--color-text)', marginBottom: 'var(--space-2)' }}>{item.label}</p>
