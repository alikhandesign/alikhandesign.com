    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <div style={{ border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)', overflow: 'hidden' }}>
        ...
        <div style={{ padding: 'var(--space-6)', minHeight: 120, display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          {simState === 'idle' && <p style={{ color: 'var(--color-text-faint)', fontSize: 'var(--font-size-sm)', fontStyle: 'italic' }}>Select an error type above. Hung and network errors stream partial content before the watchdog fires.</p>}
          ...
          {displayedText && (
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text)', lineHeight: 'var(--line-height-loose)' }}>
              {displayedText}
        ...
        {simState === 'generating' && countdown > 0 && (
          <div style={{ padding: 'var(--space-2) var(--space-4)', borderTop: '1px solid var(--color-border)', background: '#FFFBEB', fontSize: 'var(--font-size-xs)', color: '#B45309' }}>
        ...
        <div style={{ borderTop: '1px solid var(--color-border)', padding: 'var(--space-3) var(--space-4)', background: 'var(--color-surface-subtle)', display: 'flex', gap: 'var(--space-3)', alignItems: 'center' }}>
          <input readOnly value="Analyze the system architecture and identify the primary failure modes in the current implementation."
            style={{ flex: 1, padding: 'var(--space-2) var(--space-3)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', fontSize: 'var(--font-size-sm)', fontFamily: 'var(--font-sans)', background: 'var(--color-surface)', color: 'var(--color-text-muted)', outline: 'none' }}
          />
          <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-faint)', whiteSpace: 'nowrap' }}>Preserved</span>
        </div>
      </div>

      {/* Per-error audit finding — shown after error triggers */}
      {simState === 'errored' && (
        <div style={{ padding: 'var(--space-3) var(--space-4)', background: 'var(--color-surface-subtle)', borderLeft: '3px solid var(--color-accent)', borderRadius: '0 var(--radius-sm) var(--radius-sm) 0' }}>
          <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', lineHeight: 'var(--line-height-normal)' }}>
            <strong style={{ color: 'var(--color-accent)', textTransform: 'uppercase', letterSpacing: 'var(--letter-spacing-md)', fontSize: 'var(--font-size-xs)' }}>Audit finding — {cfg.title} —</strong>{' '}{cfg.finding}
          </p>
        </div>
      )}
    </div>
  ...
  const states = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      {(Object.entries(ERROR_CONFIG) as Array<[ErrorType, typeof ERROR_CONFIG[ErrorType]]>).map(([type, c]) => (
        <div key={type} style={{ padding: 'var(--space-4)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)' }}>
          <p style={{ fontSize: 'var(--font-size-sm)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--color-text)', marginBottom: 'var(--space-2)' }}>{c.title}</p>
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-muted)', lineHeight: 'var(--line-height-normal)', marginBottom: 'var(--space-3)' }}>{c.message} Recovery: {c.cta}{c.secondaryCta ? ` / ${c.secondaryCta}` : ''}.</p>
        </div>
      ))}
    </div>
  )
