  ...
  return (
    <div style={{ maxWidth: 'var(--max-w)', margin: '0 auto' }}>
      ...
      <div className="page-header">
        ...
        <p style={{ fontSize: 'var(--text-base)', color: 'var(--color-text-muted)', lineHeight: 'var(--line-height-normal)', maxWidth: 640 }}>
          {problem}
        </p>
      </div>
      ...
      <div className="next-project" style={{ borderTop: '1px solid var(--color-border)', marginTop: 0 }}>
        <Link href={`/patterns/${prev.slug}`} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 'var(--space-1)' }}>
          ...
          <span style={{ fontSize: 'var(--text-base)', color: 'var(--color-text)', fontWeight: 'var(--font-weight-medium)' }}>{prev.title}</span>
        </Link>
        <Link href={`/patterns/${next.slug}`} style={{ textDecoration: 'none', display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', alignItems: 'flex-end' }}>
          ...
          <span style={{ fontSize: 'var(--text-base)', color: 'var(--color-text)', fontWeight: 'var(--font-weight-medium)' }}>{next.title}</span>
        </Link>
      </div>
