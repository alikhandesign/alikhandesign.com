...
export default function SourceAttributionPage() {
  ...
  const uncitedMarker = (
    <span title="No source available — claim drawn from parametric memory"
      style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 18, height: 18, borderRadius: '50%', background: 'var(--color-surface-subtle)', border: '1px solid var(--color-border-mid)', fontSize: 'var(--font-size-badge)', color: 'var(--color-text-muted)', cursor: 'help', marginLeft: 3, verticalAlign: 'middle' }}>!</span>
  )
  ...
  const demo = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-6)' }}>
      ...
      <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-muted)', lineHeight: 'var(--line-height-normal)' }}>
        Click any numbered citation to open the Source Inspector. The <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: 18, height: 18, borderRadius: '50%', background: 'var(--color-surface-subtle)', border: '1px solid var(--color-border-mid)', fontSize: 'var(--font-size-badge)', color: 'var(--color-text-muted)', verticalAlign: 'middle' }}>!</span> marker indicates a claim with no external source.
      </p>
      <div style={{ display: 'grid', gridTemplateColumns: openSource ? '1fr 260px' : '1fr', gap: 0, border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', overflow: 'hidden', transition: 'grid-template-columns 0.25s ease' }}>
        <div style={{ padding: 'var(--space-6)', borderRight: openSource ? '1px solid var(--color-border)' : 'none' }}>
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text)', lineHeight: 2 }}>
        ...
        {openSource && activeSourceData && (
          <div style={{ background: 'var(--color-surface-subtle)', padding: 'var(--space-4)', display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-1)' }}>
              <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', letterSpacing: 'var(--letter-spacing-md)', textTransform: 'uppercase', fontWeight: 'var(--font-weight-medium)' }}>Source inspector</p>
              <button onClick={() => setOpenSource(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', fontSize: 'var(--font-size-md)', lineHeight: 1, padding: 0 }} aria-label="Close source inspector">×</button>
            </div>
            {SOURCES.map(s => (
              <div key={s.id} style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
                <button onClick={() => setOpenSource(s.id)} style={{ padding: 'var(--space-3)', borderRadius: s.id === openSource ? 'var(--radius-sm) var(--radius-sm) 0 0' : 'var(--radius-sm)', border: `1px solid ${s.id === openSource ? 'var(--color-accent)' : 'var(--color-border)'}`, borderBottom: s.id === openSource ? 'none' : `1px solid ${s.id === openSource ? 'var(--color-accent)' : 'var(--color-border)'}`, background: s.id === openSource ? 'var(--color-accent-bg)' : 'var(--color-surface)', textAlign: 'left', cursor: 'pointer', fontFamily: 'var(--font-sans)', transition: 'all var(--transition-base)', width: '100%' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 'var(--space-2)', marginBottom: 'var(--space-1)' }}>
                    <span style={{ width: 18, height: 18, borderRadius: '50%', background: s.id === openSource ? 'var(--color-accent)' : 'var(--color-text-mid)', color: '#fff', fontSize: 'var(--font-size-badge)', fontWeight: 700, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{s.id}</span>
                    <span style={{ fontSize: 'var(--font-size-xs)', fontWeight: 'var(--font-weight-semibold)', color: 'var(--color-text)' }}>{s.label}</span>
                  </div>
                  <div style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', marginBottom: 'var(--space-1)' }}>{s.domain}</div>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <span style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-faint)' }}>{s.date}</span>
                ...
                {s.id === openSource && (
                  <div style={{ padding: 'var(--space-3)', background: 'var(--color-surface)', borderRadius: '0 0 var(--radius-sm) var(--radius-sm)', border: '1px solid var(--color-accent)', borderTop: '1px solid var(--color-border)' }}>
                    <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text-muted)', letterSpacing: 'var(--letter-spacing-md)', textTransform: 'uppercase', fontWeight: 'var(--font-weight-medium)', marginBottom: 'var(--space-2)' }}>Relevant excerpt</p>
                    <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--color-text)', lineHeight: 'var(--line-height-normal)', fontStyle: 'italic' }}>&ldquo;{s.excerpt}&rdquo;</p>
                  </div>
                )}
              </div>
            ))}
  ...
  const states = (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
      {[
        ...
        <div key={item.label} style={{ display: 'flex', alignItems: 'flex-start', gap: 'var(--space-4)', padding: 'var(--space-4)', border: '1px solid var(--color-border)', borderRadius: 'var(--radius-sm)', background: 'var(--color-surface)' }}>
          <span style={{ display: 'inline-flex', padding: '3px 10px', borderRadius: 'var(--radius-sm)', background: item.bg, border: `1px solid ${item.border}`, fontSize: 'var(--font-size-xs)', fontWeight: 'var(--font-weight-semibold)', color: item.color, whiteSpace: 'nowrap' }}>{item.label}</span>
          <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--color-text-muted)', lineHeight: 'var(--line-height-normal)' }}>{item.desc}</p>
        </div>
      ))}
    </div>
  )
  ...
  return (
    <div style={{ maxWidth: 680, display: 'flex', flexDirection: 'column', gap: 'var(--space-8)' }}>
      {[
        ...
        { label: 'Prescription', text: 'Four requirements: consistency (cite all comparable claims or none), inline connection (each citation badge links a specific claim to a specific source), accessibility (source content visible in panel without leaving the interface), and resolution (citations go to specific excerpts, not homepages). Source Inspector slides in on citation click. Note: the resolution requirement assumes an evidentiary citation system — one proving a specific factual claim, which describes all six audited products. A navigational citation system, designed to invite deeper exploration rather than prove a claim, may correctly resolve to a full page rather than a specific excerpt. This distinction was discovered by testing this pattern against the portfolio chat assistant on this site, which uses citations navigationally.' },
