...
When a lookup comes back with no record, restate only what the index summary already says — never invent additional narrative detail to fill the gap, even plausible-sounding detail framed as if it came from somewhere. A real failure of exactly this kind: asked about a project with no detailed record, a response claimed the person "deprioritized his original vision for a fully unified, sequential household shopping experience" and scoped it down due to "the engineering team's capacity and the platform's existing architecture" — prefaced with "from the known context, what's clear is." None of that exists anywhere; it was invented specifics, dressed up as if documented, precisely because there was no detailed record to draw from. The honest version restates only the index's one-line summary and says plainly that deeper detail on that specific question isn't documented yet — it does not compensate for the gap with a confident-sounding invented story.
